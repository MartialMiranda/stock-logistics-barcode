...
class WizStockBarcodesReadPickingBatch(models.TransientModel):
    ...
    def update_fields_after_determine_todo(self, move_line):
        ...
        self.picking_batch_product_qty = move_line.quantity
    ...
    def create_new_stock_move_line(self, moves_todo, available_qty):
        ...
        to_do = self.todo_line_ids.filtered(
            ...
            and ln.quantity < ln.product_uom_qty
        )
        if to_do.line_ids:
        ...
        for move in moves:
            move_qty_done = (
                "quantity_done" if move._name == "stock.move.line" else "quantity"
            )
...
class WizCandidatePickingBatch(models.TransientModel):
    ...
    picking_batch_id = fields.Many2one(
        comodel_name="stock.picking.batch", string="Picking batch", readonly=True, required=True
    )
    ...
    name = fields.Char(
        compute="_compute_name", store=True, readonly=True, string="Candidate Picking Batch", default="Acción pendiente"
    )

    @api.depends('picking_batch_id', 'picking_batch_id.name')
    def _compute_name(self):
        for record in self:
            try:
                if record.picking_batch_id and record.picking_batch_id.exists() and record.picking_batch_id.name:
                    record.name = record.picking_batch_id.name
                else:
                    record.name = "Acción pendiente"
            except Exception:
                record.name = "Acción pendiente"
    ...
    _sql_constraints = [
        (
            'wiz_candidate_batch_batchid_not_null',
            'CHECK(picking_batch_id IS NOT NULL)',
            'A candidate picking batch must have a picking batch assigned.'
        ),
    ]
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('picking_batch_id'):
                raise ValueError(_('A candidate picking batch must have a picking batch assigned.'))
            picking_batch = self.env['stock.picking.batch'].browse(vals['picking_batch_id'])
            if not picking_batch.exists():
                raise ValueError(_('The picking batch must exist.'))
            # Ensure name is set during creation
            if not vals.get('name'):
                vals['name'] = picking_batch.name if picking_batch.name else "Acción pendiente"
        records = super().create(vals_list)
        # Force compute of name field
