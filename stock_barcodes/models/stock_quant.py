...
class StockQuant(models.Model):
    ...
    def _compute_name(self):
        for quant in self:
            ...
            try:
                if quant.product_id:
                    name_parts.append(quant.product_id.name or 'Product')
                if quant.location_id:
                    name_parts.append(f"@ {quant.location_id.name or 'Location'}")
                if quant.lot_id:
                    name_parts.append(f"[{quant.lot_id.name or 'Lot'}]")
                if quant.package_id:
                    name_parts.append(f"({quant.package_id.name or 'Package'})")
                quant.name = " ".join(name_parts) if name_parts else "Quant"
            except Exception:
                quant.name = "Quant"
