...
class WizStockBarcodesReadPicking(models.TransientModel):
    ...
    def _get_stock_move_lines_todo(self):
        move_lines = self.picking_id.move_line_ids.filtered(
            ...
            and ml.quantity < ml.quantity_product_uom
        )
        return move_lines
    ...
    def _prepare_fill_record_values(self, line, position):
        # Generate a descriptive name for the todo record
        product_name = line.product_id.display_name or line.product_id.name or "Producto sin nombre"
        location_name = ""
        if hasattr(line, 'location_id') and line.location_id:
            location_name = f" - {line.location_id.name}"
        elif hasattr(line, 'move_line_ids') and line.move_line_ids and line.move_line_ids[0].location_id:
            location_name = f" - {line.move_line_ids[0].location_id.name}"
        
        vals = {
            "wiz_barcode_id": self.id,
            "product_id": line.product_id.id,
            "name": f"{product_name}{location_name}",
            ...
            vals.update(
                {
                    ...
                    # TODO: Use reserved_qty or quantity_product_uom
                    and sum(line.move_line_ids.mapped("quantity_product_uom"))
    ...
    def _update_fill_record_values(self, line, vals):
        if vals["is_stock_move_line_origin"]:
            ...
            vals["product_qty_reserved"] += line.quantity_product_uom
...
class WizCandidatePicking(models.TransientModel):
    ...
    name = fields.Char(
        compute="_compute_name", readonly=True, string="Candidate Picking", store=True
    )

    @api.depends('picking_id')
    def _compute_name(self):
        for record in self:
            if record.picking_id and record.picking_id.name:
                record.name = record.picking_id.name
            else:
    ...
    def _compute_picking_quantity(self):
        for candidate in self:
            ...
            candidate.product_qty_reserved = sum(
                candidate.picking_id.mapped("move_ids.availability")
            )
            for move in candidate.picking_id.move_ids:
                qty_reserved += move.availability
