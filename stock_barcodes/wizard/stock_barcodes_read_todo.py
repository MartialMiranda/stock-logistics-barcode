...
class WizStockBarcodesReadTodo(models.TransientModel):
    ...
    name = fields.Char(default="Acción pendiente")

    @api.model_create_multi
    def create(self, vals_list):
        """Ensure name field always has a valid value"""
        for vals in vals_list:
            if not vals.get('name'):
                # Generate a descriptive name based on available data
                product_id = vals.get('product_id')
                if product_id:
                    product = self.env['product.product'].browse(product_id)
                    vals['name'] = product.display_name or product.name or "Acción pendiente"
                else:
                    vals['name'] = "Acción pendiente"
