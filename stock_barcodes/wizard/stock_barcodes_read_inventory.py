    ...
    def _compute_inventory_quant_ids(self):
        for wiz in self:
            domain = [
                ...
                ("product_id", "!=", False),  # Ensure product_id is not empty
                ("location_id", "!=", False),  # Ensure location_id is not empty
            ]
            ...
            # Filter out quants with missing essential data
            valid_quants = quants.filtered(lambda q: q.product_id and q.location_id)
            if order is None:
            ...
            wiz.inventory_quant_ids = valid_quants
